...
const create = async (req, res, next) => {
  try {
    ...
    const populated = await operacion.populate(['sesion', 'par', 'timeframeLiquidez', 'timeframeObjetivo']);
...
const update = async (req, res, next) => {
  try {
    const operacion = await Operacion.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!operacion) {
      return res.status(404).json({ message: 'Operación no encontrada' });
    }
    const populated = await operacion.populate(['sesion', 'par', 'timeframeLiquidez', 'timeframeObjetivo']);
